...
Anyshift is a software written in Python. It uses pygame joystick module to read inputs from the selected device, applies the shifter to sequential logic changes and send keyboard presses so you can use your H-Shifter with your favorite old racing game. It can also write gear selected directly to memory if the adrress is known. Also supports a 7 segment display gear indicator.
